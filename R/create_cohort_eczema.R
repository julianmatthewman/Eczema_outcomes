...
create_cohort_eczema <- function(main_cohort, eventdata, metadata) {
...
for (i in seq_along(eventdata)) {
    tmerged <- tmerge(tmerged, eventdata[[i]], id=patid, time_dep_cov=tdc(start))
    tmerged[metadata$name[i]] <- tmerged$time_dep_cov
}
